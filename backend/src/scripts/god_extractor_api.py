...
# -*- coding: utf-8 -*-
"""
GOD Extractor API - Versión para uso desde el backend Node.js
Recibe un archivo TXT por argumento, extrae transacciones y las guarda en PostgreSQL.
Imprime resultado en JSON a stdout.
"""
...
def guardar_en_postgres(transferencias, borrar_existentes=False):
    ...
    try:
        ...
        insert_query = sql.SQL('''
            INSERT INTO transferencias (fecha, ref_origen, prefijo, importe, tipo, nombre, ci)
            VALUES %s
            ON CONFLICT (fecha, ref_origen, importe, tipo) DO NOTHING
        ''')
...
def buscar_ci_general(bloque_original, bloque):
    ...
    for pattern in ci_patterns:
        try:
            matches = re.findall(pattern, bloque_original, re.IGNORECASE)
            for match in matches:
                if isinstance(match, tuple):
                    ci_candidate = ''.join(match)
                    if len(ci_candidate) > 11:
                        ci_candidate = ci_candidate[:11]
                else:
                    ci_candidate = match
                if (len(ci_candidate) == 11 and
                        ci_candidate != '00000000000' and
                        not re.match(r'(\d)\1{10}', ci_candidate) and
                        re.match(r'^\d{11}$', ci_candidate)):
                    ci = ci_candidate
                    break
            if ci:
                break
    ...
    if not ci:
        for pattern in [r'CI\s*[:=]', r'C\.I\.\s*[:=]', r'C[\s\r\n]+I\s*[:=]']:
            for m in re.finditer(pattern, bloque_original, re.IGNORECASE):
                context = bloque_original[m.end():m.end()+100]
                dm = re.search(r'[\d\s\r\n]+', context)
                if dm:
                    digits = ''.join(re.findall(r'\d+', dm.group(0)))
                    if len(digits) >= 11:
                        cand = digits[:11]
                        if (cand != '00000000000' and
                                not re.match(r'(\d)\1{10}', cand) and
                                re.match(r'^\d{11}$', cand)):
                            ci = cand
                            break
            if ci:
                break
    ...
    if not ci:
        bloque_limpio = re.sub(r'\s+', ' ', bloque.replace('\n', ' ').replace('\r', ' '))
        for pattern in [r'CI\s*[:=]\s*(\d{11})\b', r'C\.I\.\s*[:=]\s*(\d{11})\b', r'C\s+I\s*[:=]\s*(\d{11})\b']:
            m = re.search(pattern, bloque_limpio, re.IGNORECASE)
            if m:
                cand = m.group(1)
                if len(cand) == 11 and cand != '00000000000' and not re.match(r'(\d)\1{10}', cand):
                    ci = cand
                    break
    return ci
...
def procesar_archivo(archivo_txt):
    ...
    patron = re.compile(
        r'(\d{1,2}/\d{1,2}/\d{4})\s+'
        r'(YR\d+|VB\d+|BD\d+|JD\d+|TR\d+|YY\d+|C5\d+|DD\d+|'
        r'KW[0-9A-Z]+|AY[0-9A-Z]+|EZ[0-9A-Z]+)\s+'
        r'((?:MM|98|KW|AY|EZ|VB|BD|JD|C5|YY|AJ|DD)[0-9A-Z]{6,})\s*'
        r'[^\r\n]*'
        r'\s(\d+\.\d{2})\s+'
        r'(Cr)',
        re.IGNORECASE
